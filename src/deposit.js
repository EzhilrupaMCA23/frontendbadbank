...
export default function Deposit() {
    ...
    useEffect(() => {
        async function fetchdata() {
            await axios.get('https://server-k676.onrender.com/data').then((item) => {
                setData(item.data);
                console.log(data);
...
// console.log(name);
        if (isNaN(deposit) || deposit < 0) {
            ...
            await axios.put(`https://server-k676.onrender.com/update/${id}`, { amount: total + deposit }).then(setTotal(total + deposit)).then(alert(`amount ${deposit} is deposited successfully !`));
...

    {
        ...
        <div id='form-div'>
            <>
                ...
                <Card>
                    <Form className="form-inline" onSubmit={submitHandler}style={{borderSizing:"10px"}}>
                        <h1>Deposit</h1>
